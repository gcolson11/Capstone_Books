...
# to tune algorithm look at user_subset method
...
st.title('Read-y Books')
st.write('Would you like to manually input book ratings or randomly generate a recommendation?')
choice = st.radio(' ', ['-', 'Input book ratings', 'Randomly generate'])

# create empty list to store pairs of book title and rating
d = []

# if user chooses to input their own data
if choice == 'Input book ratings':
    # let's user choose number of books they'd like to rate
    j = st.number_input('How many books would you like to rate?', value=5, min_value=1, max_value=10, step=1)

    st.write("In the sidebar on the left, search for a book that you've read and give it a rating:")
...
class Collaborative_Filtering:
    ...
    # make dataframe with only essential columns, remove missing values, convert year column to type int
    def books_cfdf(self):
        self.books_cf = self.books[['id', 'title', 'authors', 'original_publication_year']]
        self.books_cf = self.books_cf.loc[self.books_cf['original_publication_year'].notna()]
    ...
    def user_subset(self):
        ...
        # limit number of users we look through
...
# wait until user changes input from default placeholder '-'
if choice == '-':
    pass
